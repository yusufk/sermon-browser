...
//Shows database statistics in the HTML comments if define('SAVEQUERIES', true); is set in wp-config.php Useful for diagnostics
...
function sb_hijack() {
	...
	if (isset($_REQUEST['podcast'])) {
		...
		include(sb_get_value('wordpress_path').'/wp-content/plugins/sermon-browser/podcast.php');
		die();
	}
...
function sb_print_url($url) {
	...
	$icon_url = sb_get_value('wordpress_url').'/wp-content/plugins/sermon-browser/icons/';
